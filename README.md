...
| **Token Efficiency** | **Zero Static Token Bloat**: Injects only a compact router directive (~1,079 tok Lite [95% reduction] / ~2,345 tok Balanced [88% reduction] vs the ~19.8k derived core-subset baseline (and ~97-99% vs the full 23-workflow set)). Full workflows are read Just-In-Time (JIT) from local files only when triggered. See [`docs/BENCHMARKS.md`](./docs/BENCHMARKS.md). |
