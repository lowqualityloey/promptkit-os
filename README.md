...
| **Token Efficiency** | **Zero Static Token Bloat**: Injects only a compact router directive (~1,105 tok Lite [94% reduction] / ~2,449 tok Balanced [88% reduction] vs the ~19.8k derived core-subset baseline (v1.6.0-dated; re-derives with workflow growth, and ~97-99% vs the full 23-workflow set)). Full workflows are read Just-In-Time (JIT) from local files only when triggered. See [`docs/BENCHMARKS.md`](./docs/BENCHMARKS.md). |
...
- **Balanced** (`--balanced` or no flag, default): full 23 workflows, 2,449 tok, Level 0-3 adaptive ceremony — teams, production
