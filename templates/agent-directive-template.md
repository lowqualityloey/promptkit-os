...
- **Search Circuit Breaker (advisory)**: One unit = one read/search call (parallel batch counts per call). Halt past **≤6 (L0/L1)/≤12 (L2/L3)** with no edit/test: HALT, ask for paths. Resets on advancing edit/test — trivial edits never reset. Read-only needs no edit.
...
- **Telemetry Cards & Single Callout**: Emit card per `$KIT_DIR_REL/protocols/telemetry-cards.md` (`[■■■■■■■■□□]`; CLI: ceiling/floor box). Max 1 callout/turn (IMPORTANT > WARNING > TIP). Suppress cards on `status-cards: off` (halts fire); silent on empty state.
