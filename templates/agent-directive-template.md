...
- **Telemetry Status Cards**: Emit the 3-line card per `$KIT_DIR_REL/protocols/telemetry-cards.md` (lazy-load on demand); suppress only when `PROMPTKIT.md` declares `status-cards: off` (default on) — halts still fire.
