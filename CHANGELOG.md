...
### Added
- **Bounded Oracle Verification (Machine-Verified Quality Gate)**: Added to `protocols/code-quality-gate.md`, `workflows/fix.md`, and `workflows/commit.md`. Agents cannot self-certify completion or emit a green status card without an executed test/build command returning `exit code 0`. Distinguishes oracle gate compliance from overall task completion (which also requires satisfying acceptance criteria and user intent). Enforces bounded auto-repair: max 2 retries before a hard halt with `> [!WARNING] Blocked: Awaiting Human Input`.
