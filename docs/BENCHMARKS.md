...
│ Baseline Static Injection: Router Directive (1,079 tok Lite / 2,401 Bal)  │
...
| :--- | :--- | ---: | ---: | :--- | :--- |
| **Lite** | `agent-directive-lite-template.md` (6 utility workflows: route, debug, commit, checkpoint, sync, profile) | 4,314 | **1,079 tok** | **95% static** | Onboarding, new users, tiny fixes |
| **Balanced** | `agent-directive-template.md` (23 workflows) | 9,604 | **2,401 tok** | **88% static** | Teams, production, default |
| **Turbo** | same as Balanced + parallel waves | 9,604 | 2,401 tok + subagents (~2x measured total (bounds model, see section 8)) | 88% static, higher total | Experimental, greenfield, accepts cost |
...
| **Smart Auto-Route & Guardrails** | ~21 | ~371 tokens | Triage rules (Fast-Path zero overhead, Anti-slop, Secrets hygiene, MCP precedence, Telemetry-cards pointer, status-cards opt-out, alias registry) |
...
| **Total Baseline Static Overhead (Balanced)** | **96 lines** | **~2,401 tokens** | **Permanent footprint in system prompt (~88% static saving vs. ~19.8k core-subset (~97% vs. full 23-file set))** |
| **Total Baseline Static Overhead (Lite)** | **~49 lines** | **~1,079 tokens** | **95% static saving, 55% saving vs Balanced** |
| **Opt-in add-on: §5a LSP diagnostics** | `workflows/review.md` step 2a (~313 tok) + Diagnostics Evidence table (~134 tok) | 3,228 | **~447 tok** | Additive only when `LSP Enabled: true`; runtime evidence capped at 150 lines | **Balanced + `pk:review` on TS repos** (Lite stays at 1,079 tok — skipped silently) |
...
PromptKit OS benchmarks both the **static footprint** (1,079 tok Lite, 2,401 tok Balanced) and the **dynamic per-task runtime context**. 
...
- JIT payload = directive 1,079-2,401 + workflow + gate (new behavior, route.md only when ambiguous)
- Lite vs Balanced: Lite uses 1,079 tok directive, Balanced uses 2,401 tok
...
| :--- | :--- | :---: | :---: | :---: | :---: |
| **`pk:fix`** | Localized bug fix (Directive + `fix.md` + `code-quality-gate.md`) | 12,861 tok | **6,713 tok** | **5,391 tok** (1,079+fix+gate) | **-48% Balanced, -58% Lite (-6,148 to -7,470 tok)** |
| **`pk:plan`** | Controlled feature planning (Directive + `plan.md` + `tech-spec` + `gate`) | 24,666 tok | **16,268 tok** | **14,946 tok** | **-34% Balanced, -39% Lite** |
| **`pk:ship`** | Release candidate & verification (Directive + `ship.md` stub + `gate`) | 24,761 tok | **11,784 tok** | **10,462 tok** | **-52% Balanced, -58% Lite (-12,977 to -14,299 tok)** |
