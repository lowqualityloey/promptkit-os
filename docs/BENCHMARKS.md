...
│ Baseline Static Injection: Router Directive (1,079 tok Lite / 2,345 Bal)  │
...
| **Balanced** | `agent-directive-template.md` (23 workflows) | 9,381 | **2,345 tok** | **88% static** | Teams, production, default |
| **Turbo** | same as Balanced + parallel waves | 9,381 | 2,345 tok + subagents (~2x measured total (bounds model, see section 8)) | 88% static, higher total | Experimental, greenfield, accepts cost |
...
| **Smart Auto-Route & Guardrails** | ~21 | ~315 tokens | Triage rules (Fast-Path zero overhead, Anti-slop, Secrets hygiene, MCP precedence, Telemetry-cards pointer, status-cards opt-out) |
...
| **Total Baseline Static Overhead (Balanced)** | **96 lines** | **~2,345 tokens** | **Permanent footprint in system prompt (~88% static saving vs. ~19.8k core-subset (~97% vs. full 23-file set))** |
| **Total Baseline Static Overhead (Lite)** | **~49 lines** | **~1,079 tokens** | **95% static saving, 54% saving vs Balanced** |
...
PromptKit OS benchmarks both the **static footprint** (1,079 tok Lite, 2,345 tok Balanced) and the **dynamic per-task runtime context**. 
...
- JIT payload = directive 1,079-2,345 + workflow + gate (new behavior, route.md only when ambiguous)
- Lite vs Balanced: Lite uses 1,079 tok directive, Balanced uses 2,345 tok
...
| :--- | :--- | :---: | :---: | :---: | :---: |
| **`pk:fix`** | Localized bug fix (Directive + `fix.md` + `code-quality-gate.md`) | 12,861 tok | **6,657 tok** | **5,391 tok** (1,079+fix+gate) | **-48% Balanced, -58% Lite (-6,204 to -7,470 tok)** |
| **`pk:plan`** | Controlled feature planning (Directive + `plan.md` + `tech-spec` + `gate`) | 24,666 tok | **16,212 tok** | **14,946 tok** | **-34% Balanced, -39% Lite** |
| **`pk:ship`** | Release candidate & verification (Directive + `ship.md` stub + `gate`) | 24,761 tok | **11,728 tok** | **10,462 tok** | **-53% Balanced, -58% Lite (-13,033 to -14,299 tok)** |
