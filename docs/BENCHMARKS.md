...
│ Baseline Static Injection: Router Directive (1,105 tok Lite / 2,441 Bal)  │
...
| **Balanced** | `agent-directive-template.md` (23 workflows) | 9,764 | **2,441 tok** | **88% static** | Teams, production, default |
| **Turbo** | same as Balanced + parallel waves | 9,764 | 2,441 tok + subagents (~2x measured total (bounds model, see section 8)) | 88% static, higher total | Experimental, greenfield, accepts cost |
...
| **Smart Auto-Route & Guardrails** | ~22 | ~411 tokens | Triage rules (Fast-Path zero overhead, Anti-slop, Secrets hygiene, Circuit Breaker, MCP precedence, Telemetry-cards pointer, status-cards opt-out, alias registry) |
...
| **Total Baseline Static Overhead (Balanced)** | **97 lines** | **~2,441 tokens** | **Permanent footprint in system prompt (~88% static saving vs. ~19.8k core-subset (~97% vs. full 23-file set))** |
...
PromptKit OS benchmarks both the **static footprint** (1,105 tok Lite, 2,441 tok Balanced) and the **dynamic per-task runtime context**. 
...
- JIT payload = directive 1,105-2,441 + workflow + gate (new behavior, route.md only when ambiguous)
- Lite vs Balanced: Lite uses 1,105 tok directive, Balanced uses 2,441 tok
...
| :--- | :--- | :---: | :---: | :---: | :---: |
| **`pk:fix`** | Localized bug fix (Directive + `fix.md` + `code-quality-gate.md`) | 12,861 tok | **6,962 tok** | **5,626 tok** (1,105+fix+gate) | **-46% Balanced, -56% Lite (-5,899 to -7,235 tok)** |
| **`pk:plan`** | Controlled feature planning (Directive + `plan.md` + `tech-spec` + `gate`) | 24,666 tok | **16,453 tok** | **15,117 tok** | **-33% Balanced, -39% Lite** |
| **`pk:ship`** | Release candidate & verification (Directive + `ship.md` stub + `gate`) | 24,761 tok | **11,969 tok** | **10,633 tok** | **-52% Balanced, -57% Lite (-12,792 to -14,128 tok)** |
