...
│ Baseline Static Injection: Router Directive (1,146 tok Lite / 2,500 Bal)  │
...
| **Balanced** | `agent-directive-template.md` (24 workflows) | 9,991 | **2,500 tok** | **87% static** | Teams, production, default |
| **Turbo** | same as Balanced + parallel waves | 9,991 | 2,500 tok + subagents (~2x measured total (bounds model, see section 8)) | 87% static, higher total | Experimental, greenfield, accepts cost |
...
| **Total Baseline Static Overhead (Balanced)** | **100 lines** | **~2,500 tokens** | **Permanent footprint in system prompt (~87% static saving vs. ~19.8k core-subset (~97% vs. full 24-file set))** |
...
PromptKit OS benchmarks both the **static footprint** (1,146 tok Lite, 2,500 tok Balanced) and the **dynamic per-task runtime context**. 
...
- JIT payload = directive 1,146-2,500 + workflow + gate (new behavior, route.md only when ambiguous)
- Lite vs Balanced: Lite uses 1,146 tok directive, Balanced uses 2,500 tok
...
| :--- | :--- | :---: | :---: | :---: | :---: |
| **`pk:fix`** | Localized bug fix (Directive + `fix.md` + `code-quality-gate.md`) | 12,861 tok | **8,069 tok** | **6,715 tok** | **-37% Balanced, -48% Lite (-4,792 to -6,146 tok)** |
| **`pk:plan`** | Controlled feature planning (Directive + `plan.md` + `tech-spec` + `gate`) | 24,666 tok | **17,864 tok** | **16,510 tok** | **-28% Balanced, -33% Lite** |
| **`pk:ship`** | Release candidate & verification (Directive + `ship.md` stub + `gate`) | 24,761 tok | **13,748 tok** | **12,394 tok** | **-44% Balanced, -50% Lite (-11,013 to -12,367 tok)** |
