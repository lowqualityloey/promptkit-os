...
5. **Close**: user says "enough for now" → Intake Record with `close_reason: user-declared-sufficient`; `size: small` / `intake-status: complete` written to `PROMPTKIT.md`.
...
| Predicted band (spec) | +2,000–4,000 one-time → **actual ~3,950: within band** |
