...
- **CPU Time Budgets**: The runtime meters actual CPU execution time, not idle wall-clock I/O time. CPU time limits are plan-dependent (e.g., historically 10ms for Free, 50ms for Standard, or higher for unbound modes; check current Cloudflare limits). Complex cryptographic operations or heavy JSON parsing must be optimized to prevent CPU timeout termination.
...
- **Subrequest Limits**: Workers have strict outbound `fetch` subrequest limits per invocation (e.g., historically 50 or 1000 depending on plan; check current Cloudflare documentation). Firing unbounded parallel HTTP requests causes subrequest limit exceptions (`Too many subrequests`).
