...
| **Product & Design Inputs** | **MVP & UX Authority**: Figma links, screenshots, `STYLE.md` / `DESIGN.md`, and tracker-board links captured by the discovery intake (`protocols/discovery-intake.md`). | Loaded during `pk:onboard` intake and consumed by `pk:plan` Step 0 and `pk:design`; unread inputs are reported to the user, never invented. |
