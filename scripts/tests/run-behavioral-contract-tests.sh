...
assert_contains "workflows/checkpoint.md" "lower and upper bounds independently" "Checkpoint preserves heuristic ranges instead of midpoint estimates"
assert_contains "workflows/checkpoint.md" "Never add measured tokens to a heuristic range" "Checkpoint keeps measured and heuristic token evidence separate"
assert_contains "workflows/checkpoint.md" "operational telemetry, not CPAC benchmark evidence" "Checkpoint distinguishes continuity telemetry from formal benchmarks"
assert_contains "templates/state-tracker-template.md" "Estimated context payload" "State template labels heuristic payload without implying provider spend"
assert_contains "templates/state-tracker-template.md" "repairs=<count or not tracked>" "State template records compact process evidence without inventing missing values"
...
echo ""
echo "📌 Scenario V: Profile Drift Audit (pk:sync vs Repository Reality — Issue #338)"
assert_contains "workflows/sync.md" "Profile Drift Audit" "Sync defines the PROMPTKIT.md drift audit section"
assert_contains "workflows/sync.md" "detection-only pass" "Sync drift audit is detection-only"
assert_contains "workflows/sync.md" "No dependency resolution" "Sync drift audit is bounded to manifest presence"
assert_contains "workflows/sync.md" "Detection is observation, not promotion" "Sync drift findings stay on the observation side of the memory-vs-policy boundary"
assert_contains "workflows/sync.md" "Never auto-rewrite user-authored sections" "Sync drift audit never auto-rewrites PROMPTKIT.md"
assert_contains "workflows/sync.md" "Clean profile is silent" "Clean profile produces no findings and no extra ceremony"

echo ""
echo "📌 Scenario W: Turbo Suitability Check (pk:profile — Issue #337)"
assert_contains "workflows/profile.md" "Turbo suitability check" "Profile workflow asks the Turbo suitability question"
assert_contains "workflows/profile.md" "zero benefit on sequential work" "Sequential workloads are warned that waves buy zero benefit"
assert_contains "workflows/profile.md" "Advisory, never blocking" "Suitability check is advisory and never blocks Turbo"
assert_contains "workflows/profile.md" "auto-waves-preflight-checklist.md" "Suitability check links the pre-flight checklist as evidence"
assert_contains "workflows/profile.md" "never restate its ticks" "Suitability check links the checklist instead of duplicating it"
assert_contains "workflows/profile.md" "still honored" "Explicit Turbo override remains honored after a no answer"
assert_contains "workflows/onboard.md" "advisory suitability check" "Onboarding runs the advisory suitability check before Turbo acknowledgement"

echo ""
echo "📌 Scenario X: Proactive Downgrade & Checkpoint Cadence Guidance (pk:route — Issue #339)"
assert_contains "workflows/route.md" "Proactive Level-Fit Check" "Router performs a proactive per-milestone level-fit check"
assert_contains "workflows/route.md" "Milestone Boundaries" "Level-fit check is anchored to milestone boundaries"
assert_contains "workflows/route.md" "Proposed, never forced" "Downgrades stay proposed, never forced"
assert_contains "workflows/route.md" "Verification is not downgraded" "Downgrade reduces records, never verification"
assert_contains "workflows/route.md" "Checkpoint Cadence by Project Size" "Router documents checkpoint cadence by project size"
...
echo ""
echo "📌 Scenario Z: Sharpened Subagent Verifier & Reviewer Briefs (Issue #390)"
assert_contains "protocols/subagent-delegation.md" "Claims-Audit [Dd]uty" "Subagent delegation defines Claims-Audit duty for reviewers and verifiers"
assert_contains "protocols/subagent-delegation.md" "re-verifying.*against the diff and recorded exit codes" "Subagent delegation requires re-verifying claims against diff and exit codes"
assert_contains "workflows/review.md" "Claims-Audit [Dd]uty" "Review workflow inherits Claims-Audit duty in dual-axis delegation"
assert_contains "workflows/review.md" "security-lens third reviewer" "Review workflow defines trigger-based security-lens third reviewer"
assert_contains "workflows/review.md" "create[s]? no approval.*authority" "Review report creates no approval or merge authority"
assert_contains "workflows/debug.md" "adversarial pass" "Debug workflow includes adversarial pass in L2+ verifier brief"
assert_contains "workflows/debug.md" "attempt to make the fix fail; try the failure mode the fix claims to close" "Debug workflow specifies attempting to break the fix"
assert_contains "workflows/debug.md" "Level 0 and Level 1.*bypass" "Debug workflow preserves L0/L1 bypass for Pattern C verification"
assert_contains "workflows/ship.md" "evidence audit" "Ship workflow requires Pattern C evidence audit before coordinator review"
assert_contains "workflows/ship.md" "tag proposals are unexecuted" "Ship evidence audit verifies tag proposals are unexecuted"
...
echo ""
echo "📌 Scenario AF: WordPress CMS Stack Playbook & Discovery (Issue #402)"
assert_contains "docs/stacks/README.md" "cms-wordpress.md" "Stacks README catalog indexes cms-wordpress playbook"
assert_contains "docs/stacks/cms-wordpress.md" "name: cms-wordpress" "WordPress playbook defines valid frontmatter name"
assert_contains "docs/stacks/cms-wordpress.md" "category: web" "WordPress playbook uses category: web"
assert_contains "docs/stacks/cms-wordpress.md" "wp-config.php" "WordPress playbook declares wp-config.php manifest activation"
assert_contains "docs/stacks/cms-wordpress.md" "composer validate" "WordPress playbook declares fast verification command"
assert_contains "docs/stacks/cms-wordpress.md" "Mandatory Prepared SQL Statements" "WordPress playbook defines prepared statements invariant"
